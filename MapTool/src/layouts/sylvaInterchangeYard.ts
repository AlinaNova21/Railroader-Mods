
import { MathUtils, Matrix4, Vector3 } from 'three'

import { AlinasMapModMixin } from '../lib/AlinasMapMod.js'
import { Graph, Id, Segment, TrackNode, TrackSpanPartEnd, idGenerator, loadHelper } from '../lib/index.js'

const UP = new Vector3(0, 1, 0)

/** @param {Graph} graph */
export default async function sylvaInterchangeYard(graph: Graph, originalTracks: Graph) {
  const area = `AN_Sylva_Interchange_Yard`

  const additionalTracks = 7

  const switchAngle = 8
  const trackLength = 500
  const node = graph.newNode(Id(`N${area}_00`), new Vector3())
  let lastNode = node
  for (let i = 0; i < additionalTracks; i++) {
    const { nid, sid } = idGenerator(`${area}_T${i}`)
    lastNode = lastNode
      .extend(nid(), sid(), 30)

    lastNode.rotation.y = -switchAngle

    const n1 = lastNode
      .extend(nid(), sid(), 20, switchAngle)

    n1.position.add(new Vector3(-1, 0, 0))
    n1.extend(nid(), sid(), trackLength - n1.position.z, 0)
  }

  const n1 = graph.nodes.get(Id(`N${area}_T0_00`)) as TrackNode
  const n2 = graph.nodes.get(Id(`N${area}_T0_01`)) as TrackNode

  n1.position.x = n2.position.x
  n1.rotation.y = 0

  // Integrate yard
  const ref = originalTracks.nodes.get(Id('Nlfg')) as TrackNode
  const in1 = originalTracks.nodes.get(Id('Nf8a')) as TrackNode
  const in2 = originalTracks.nodes.get(Id('Nmjn')) as TrackNode
  graph.nodes.set(in1.id, in1)
  graph.nodes.set(in2.id, in2)

  graph.nodes.delete(node.id) // Remove starter
  const seg = graph.segments.get(Id(`S${area}_T0_00`)) as Segment
  seg.startId = in1.id

  const matrix = new Matrix4()
  const positionOffset = new Vector3(-7, 0, 75)
  matrix.makeRotationAxis(UP, MathUtils.degToRad(ref.rotation.y))

  graph.nodes.forEach((node, id) => {
    if (id.startsWith(`N${area}`)) {
      node.position
        .add(positionOffset)
        .applyMatrix4(matrix)
        .add(ref.position)
      node.rotation.y += ref.rotation.y
    }
  })

  graph.segments.forEach((segment, id) => {
    if (id.startsWith(`S${area}`)) {
      segment.groupId = area
    }
  })

  const span1 = graph.newSpan(Id(`P${area}_00`), {
    segmentId: Id("S3ns"),
    end: TrackSpanPartEnd.Start,
    distance: 0
  }, {
    segmentId: Id("S3ns"),
    end: TrackSpanPartEnd.End,
    distance: 0
  })

  const mixin: AlinasMapModMixin = {
    items: {
      [area]: {
        identifier: area,
        name: 'Sylva Interchange Yard',
        groupIds: [area],
        description: 'A yard that can be useful for organizing west bound trains and storing cars if the Interchange is filled to capacity.',
        deliveryPhases: [{
            cost: 2000,
            deliveries: [
              loadHelper('ballast', 4, 'GB*'),
              loadHelper('gravel', 12, 'GB*'),
            ]
          },
          {
            cost: 2000,
            deliveries: [
              loadHelper('gravel', 2, 'GB*'),
              loadHelper('ties', 8, 'GB*'),
              loadHelper('rails', 6, 'FM*'),
            ]
          }],
        prerequisiteSections: ['s1'],
        area: 'sylva',
        trackSpans: [span1.id]
      }
    }
  }

  return {
    name: mixin.items[area].name,
    mixins: {
      alinasMapMod: mixin
    }
  }
  // graph.newSegment(`N${area}_L0`, in1, inNodes[0])
  // graph.newSegment(`N${area}_L1`, in2, outNodes[0])
}